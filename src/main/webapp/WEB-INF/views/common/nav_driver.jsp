					...
					<ul class="nav navbar-nav float-lg-right navbar-right">
						...
						<c:if test="${!empty sessionScope.loginUser}">
							<c:if test='${loginUser.mSort eq ("관리자")}'>
								<li class="nav-item">
									<button class="btn btn-default navbar-btn adminPage" style="background: #F15F5F; color: white;">관리자 홈</button>
								</li> 
								<li class="nav-item">
									<button class="btn btn-default navbar-btn userPage" style="background: #F15F5F; color: white;">일반 홈</button>
								</li>
								<li class="nav-item">
								<button class="btn btn-default navbar-btn chatPage" onclick="location.href='adminChat.ch'" style="background: none;border: none; position:relative;">
